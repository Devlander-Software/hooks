...
export type AuthAction =
  ...
  | {
      type: "INITIALIZE"
      payload: { session: string | null; isGuest: boolean }
    }
  | {
      type: "AUTH_SUCCESS"
      payload: { session: string; actionType: "login" | "register" }
    }
  | { type: "LOGOUT_SUCCESS" }
  ...
  switch (action.type) {
    ...
    case "AUTH_SUCCESS":
      return {
        ...state,
        isLoggingIn:
          action.payload.actionType === "login" ? false : state.isLoggingIn,
        isRegistering:
          action.payload.actionType === "register"
            ? false
            : state.isRegistering,
        session: action.payload.session,
        isUser: true,
