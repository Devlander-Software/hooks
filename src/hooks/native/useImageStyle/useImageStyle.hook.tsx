...
export const useImageStyle = (
  ...
  newStyles?: ImageStyle,
): ImageStyle => {
  const combinedStyleProps = useMemo(() => {
    const initialStyle =
      imageStyleProps && !isEmpty(imageStyleProps) ? imageStyleProps : {}

    const additionalStyle = newStyles && !isEmpty(newStyles) ? newStyles : {}

    return { ...initialStyle, ...additionalStyle }
  }, [imageStyleProps, newStyles])
