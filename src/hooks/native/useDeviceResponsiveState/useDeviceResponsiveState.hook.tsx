...
export const useDeviceResponsiveState = (
  ...
  const viewportCategory = useMemo(() => {
    ...
    const { extraSmall, small, medium, large, extraLarge } = appliedMediaQueryConfig;

    if (extraSmall && width <= (extraSmall.maxWidth || 0))
      return ViewportCategory.ExtraSmall;
    if (small && width >= (small.minWidth || 0) && width <= (small.maxWidth || Infinity))
      return ViewportCategory.Small;
    if (medium && width >= (medium.minWidth || 0) && width <= (medium.maxWidth || Infinity))
      return ViewportCategory.Medium;
    if (large && width >= (large.minWidth || 0) && width <= (large.maxWidth || Infinity))
